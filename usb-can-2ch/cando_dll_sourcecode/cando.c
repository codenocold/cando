...
bool __stdcall DLL cando_set_timing(cando_handle hdev, uint16_t ch, cando_bittiming_t *timing)
{
    // TODO ensure device is open, check channel count..
    cando_device_t *dev = (cando_device_t*)hdev;
    return cando_ctrl_set_bittiming(dev, ch, timing);
}

bool __stdcall DLL cando_start(cando_handle hdev, uint16_t ch, uint32_t mode)
{
    // TODO ensure device is open, check channel count..
    cando_device_t *dev = (cando_device_t*)hdev;
    mode |= (CANDO_MODE_HW_TIMESTAMP);
    return cando_ctrl_set_device_mode(dev, ch, CANDO_DEVMODE_START, mode);
}

bool __stdcall DLL cando_stop(cando_handle hdev, uint16_t ch)
{
    // TODO ensure device is open, check channel count..
    cando_device_t *dev = (cando_device_t*)hdev;
    return cando_ctrl_set_device_mode(dev, ch, CANDO_DEVMODE_RESET, 0);
}
