...
extern led_data_t hLED0, hLED1, hLED2;
...
static uint8_t USBD_GS_CAN_EP0_RxReady(USBD_HandleTypeDef *pdev)
{
    ...
    switch (req->bRequest) {
		...
		case GS_USB_BREQ_IDENTIFY:
    		...
    		if (param_u32) {
    			led_run_sequence(ch->led, led_identify_seq, -1);
    		} else {
        		led_set_mode(ch->led, can_is_enabled(ch) ? led_mode_normal : led_mode_off);
    		}
    		break;
    	...
    	case GS_USB_BREQ_MODE:
    		if (req->wValue < NUM_CAN_CHANNEL) {
				...
				if (mode->mode == GS_CAN_MODE_RESET) {
					can_disable(ch);
					led_set_mode(ch->led, led_mode_off);
					...
					if(mode->flags & GS_CAN_MODE_NO_ECHO_BACK) {
						gEchoBackEnable = false;
					}else {
						gEchoBackEnable = true;
					}
					can_enable(ch,
					...
					led_set_mode(ch->led, led_mode_normal);
				}
			}
    		break;
