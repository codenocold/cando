...
{ 
	...
	HAL_PCDEx_SetRxFiFo(&hpcd_USB_OTG_FS, 0x80);		// EP OUT(Rx) FIFO size	
	HAL_PCDEx_SetTxFiFo(&hpcd_USB_OTG_FS, 0, 0x30);		// EP0 IN(Tx) FIFO size
	HAL_PCDEx_SetTxFiFo(&hpcd_USB_OTG_FS, 1, 0x80);		// EP1 IN(Tx) FIFO size
