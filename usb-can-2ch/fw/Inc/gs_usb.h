...
#define GSUSB_ENDPOINT_IN          0x81		/* EP1 for data IN (H<-D) */
#define GSUSB_ENDPOINT_OUT         0x02		/* EP2 for data OUT (H->D) */
...
#define GS_CAN_MODE_NO_ECHO_BACK				(1<<8)
...
__packed struct gs_host_config {
	u32 byte_order;
};
...
__packed struct gs_device_config {
	u8 reserved1;
	u8 reserved2;
	u8 reserved3;
	u8 icount;
...
__packed struct gs_device_mode {
	u32 mode;
	u32 flags;
};
...
__packed struct gs_device_bt_const {
	u32 feature;
	u32 fclk_can;
	u32 tseg1_min;
	u32 tseg1_max;
